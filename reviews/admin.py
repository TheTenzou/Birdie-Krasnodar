...
class ReviewVoteAdmin(admin.TabularInline):
    model = ReviewVote
    extra = 0
...
class ReviewAdmin(admin.ModelAdmin):
    ...
    inlines = [ ReviewVoteAdmin, ]

    def get_form(self, request, obj=None, **kwargs):
        form = super().get_form(request, obj, **kwargs)
        form.base_fields['user'].label = 'Пользователь'
        form.base_fields['restaurant'].label = 'Ресторан'
        # form.base_fields['up_vote_users'].label = 'Пользователи проголосовавшие за'
        # form.base_fields['down_vote_users'].label = 'Пользователи проголосовавшие против'
